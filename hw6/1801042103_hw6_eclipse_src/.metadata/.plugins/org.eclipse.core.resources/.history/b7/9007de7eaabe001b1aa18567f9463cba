...
public class Customer extends User {
	...
	public ArrayList<Product> searchTrader(String trader) {
		ArrayList<Product> te = new ArrayList<Product>();
		String temp;
		int count = 0;
		try {
			brc = new BufferedReader(new FileReader("data/product.txt"));
			while ((temp = brc.readLine()) != null) {
				...
				if (token[6].equals(trader)) {
					te.add(count, new Product(token[0], token[1], token[2], Integer.parseInt(token[3]),
							Integer.parseInt(token[4]), token[5], token[6]));
					count++;
				}
			}
	...
	/**
	 * This method provide order that doing order Customer.If there is a product, it
	 * deletes the product in the source file. It is also written in order.txt.
	 * 
	 * @param prdct   Product
	 * @param cusName String
	...
	public boolean order(Product prdct, String cusName) {
		...
		try {
			brc = new BufferedReader(new FileReader("data/product.txt"));
			bwo = new BufferedWriter(new FileWriter("data/order.txt",true));
			while ((temp = brc.readLine()) != null && flag==0) {
				String[] token = temp.split(";");
