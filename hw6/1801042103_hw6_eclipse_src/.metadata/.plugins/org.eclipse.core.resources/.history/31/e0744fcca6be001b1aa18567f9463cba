 ...
 * allows me to keep categories in a tree.
 * 
 * @author ozan.geckin
 *
 */
public class Parser {
	...
	private BufferedWriter bwto;
	/** Hashtable holding the user name */
	private Hashtable<Integer,String> tre = new Hashtable<Integer,String>();
	...
	/**
	 * This method create user.txt and order.txt file and write title
	 */
	public void defaultUser() {
		try {
			bwto = new BufferedWriter(new FileWriter("data/order.txt")); 
			bwt = new BufferedWriter(new FileWriter("data/user.txt"));
			bwt.write("id;name;password;role");
			bwt.write("\n");
			bwto.write("product_id;from;to;status");
			bwto.write("\n");
			bwt.close();
			bwto.close();
		} catch (IOException e) {
